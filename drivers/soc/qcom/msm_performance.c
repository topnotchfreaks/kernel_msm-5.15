...
static void activate_splh_notif(void)
{
	...
	if (splh_notif)
		ret = plh_ops->start_plh(plh_handle, splh_notif, PERF_LOCK_SCROLL);
	else
...
static void activate_lplh_notif(void)
{
	...
	if (lplh_notif)
		ret = plh_ops->start_plh(plh_handle, lplh_notif, PERF_LOCK_LAUNCH);
	else
...
#if IS_ENABLED(CONFIG_QTI_PLH_SCMI_CLIENT)
