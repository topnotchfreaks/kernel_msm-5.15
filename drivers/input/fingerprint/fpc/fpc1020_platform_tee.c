...
static int fpc_proc_open(struct inode *inode, struct file *file)
{
...
static int fpc1020_probe(struct platform_device *pdev)
{
	...
	*/
	...
	vccgpio = rc;
	if (rc < 0) {
		dev_err(dev, "failed to get fpc,gpio_vdd_for_plsensor\n");
		return rc;
	}
	rc = gpio_request(vccgpio, "gpio_vdd");
	if (rc) {
			dev_err(dev, "Failed to request gpio_vdd\n");
			return -EPERM;
		}
	gpio_direction_output(vccgpio,1);
	dev_info(dev, "fpc power for plsensor successfully\n");
	//add powered for plsensor
#ifndef CONFIG_FPC_COMPAT
	rc = fpc1020_request_named_gpio(fpc1020, "fpc,gpio_irq",
			&fpc1020->irq_gpio);
