...
static int gf_proc_open(struct inode *inode, struct file *file)
{
...
#endif
{
	...
	pr_info("there are goodix fingerprint ,so pwr gpio free\n");
	*/
	if (gf_parse_dts(gf_dev))
		goto error_hw;

	/*int rc = 0;
	rc = vreg_setup(gf_dev, "", true);
	if (rc)
		return rc;*/
...
*/
	...
	netlink_init();
#endif
	pr_info("goodix fingerprint fod init status = 0x%x\n", status);
	return 0;
}
...
module_exit(gf_exit);
MODULE_SOFTDEP("pre: fpc1020_platform_tee");
